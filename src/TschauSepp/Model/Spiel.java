...
public class Spiel {
     ...
     */
    public Spiel(Spieler spieler1, Spieler spieler2, Spieler spieler3, Spieler spieler4, int maximalePunkte) {
    ...
    public void pruefeKarteGeben(Spieler spielerID) {
        ...
        if(getAnzahlSpielerKarten(spielerID) < 17){
    ...
    public void pruefeKarteGeben2(Spieler spielerID) {
        ...
        if(getAnzahlSpielerKarten(spielerID) >= 16){
            if(getAnzahlSpielerKarten(spielerID) < 17){
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
            }
        }else{
    ...
    public void pruefeKarteGeben4(Spieler spielerID) {
        ...
        //kontrollier das der Spieler maxmimal 18 Karten bekommt
        if(getAnzahlSpielerKarten(spielerID) >=14){
            if(getAnzahlSpielerKarten(spielerID) < 15){
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
            }else if(getAnzahlSpielerKarten(spielerID) < 16){
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
            }else if(getAnzahlSpielerKarten(spielerID) < 17){
                getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
            }
        }else{
            getSpielerKarten1(spielerID).add(kartenstapel.gebeKarte());
    ...
    public void pruefeKarteLegen(Spieler spielerID, Karte karte) throws InvalideSymbolException, InvalideZahlException {
        ...
        if (getSpielerKarten1(getAktuellerSpieler()).size() == 2 && getAktuellerSpieler().getHatTschau() == false) {
            JLabel nachricht = new JLabel(getAktuellerSpieler().getName() + " hat vergessen Tschau zu sagen und hat zwei Strafkarten erhalten.");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
            ...
            JLabel nachricht = new JLabel(getAktuellerSpieler().getName() + " hat vergessen Sepp zu sagen und hat vier Strafkarten erhalten.");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
        ...
        if (!istZugValide(karte)) {
            if (karte.getSymbol() != validesSymbol) {
                JLabel nachricht = new JLabel("Die Karte kann nicht gespielt werden!");
                nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
                JOptionPane.showMessageDialog(null, nachricht);
        ...
        if (hatKeineKarten(this.spielerID.get(aktuellerSpieler))) {
            JLabel nachricht = new JLabel(getAktuellerSpieler().getName() + " hat die Runde gewonnen!");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
        ...
        if (karte.getZahl() == Karte.Zahl.acht) {
            JLabel nachricht = new JLabel("Der nächste Spieler wurde übersprungen!");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
            naechsterSpielerBestimmten();
        }
        if (karte.getZahl() == Karte.Zahl.zehn) {
            JLabel nachricht = new JLabel("Die Richtung wurde geändert!");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
            spielRichtung ^= true;
            if (spielRichtung == false) {
        ...
        if (karte.getZahl() == Karte.Zahl.sieben) {
            JLabel nachricht = new JLabel(getAktuellerSpieler().getName() + " muss 2 Karten aufnehmen");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
            pruefeKarteGeben2(getAktuellerSpieler());
        }
        if (karte.getZahl() == Karte.Zahl.ass) {
            ...
            JLabel nachricht = new JLabel(getAktuellerSpieler().getName() + " hat ein Ass gelegt und muss nochmal eine Karte spielen");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
        }
    }
    ...
    public void punkteZaehlen(Spieler spieler) {
            ...
            if (l != 0) {
                for (int i = 0; i < l; i++) {
                    if (getSpielerKarte(spielerID.get(m), i).getZahl().equals(Karte.Zahl.sechs)) {
                        ...
                        punkte = punkte + 3;
                    } else if (getSpielerKarte(spielerID.get(m), i).getZahl().equals(Karte.Zahl.könig)) {
                        punkte = punkte + 4;
        ...
        if (spieler.getPunktzahl() < maximalePunkte) {
            JLabel nachricht = new JLabel("Eine neue Runde wird gestartet");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
            rundeReseten();
        } else {
            JLabel nachricht = new JLabel(spieler.getName() + " hat das Spiel gewonnen!");
            nachricht.setFont(new Font("SansSerif", Font.BOLD, 25));
            JOptionPane.showMessageDialog(null, nachricht);
            System.exit(0);
        }
